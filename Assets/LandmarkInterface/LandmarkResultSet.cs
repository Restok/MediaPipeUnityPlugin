...
namespace LandmarkInterface
{
  public class LandmarkResultSet : MonoBehaviour
  {
    ...
    private LandmarkListFilter PoseFilter;
    ...
    public NormalizedLandmarkList PoseLandmarks
    {
      ...
      set
      {
        if (UseFilter)
        {
          PoseFilter.CorrectAndPredict(value);
        }
        poseLandmarks = value;
      }
    }
    ...
    private void Awake()
    {
      PoseFilter = new LandmarkListFilter(FilterTimeInterval, FilterNoise, DisplacementLimit);
    ...
    private void Update()
    {
      ...
      PoseFilter.UpdateFilterParameter(FilterTimeInterval, FilterNoise, DisplacementLimit);
    }
    ...
    public void UpdateLandmark(LandmarkType landmarkType, NormalizedLandmarkList landmarkList)
    {
      ...
      else if (landmarkType == LandmarkType.Pose)
      {
    ...
    public List<Vector3> GetLandmarks(LandmarkType landmarkType)
    {
      if (UseFilter)
      {
        ...
        {
          return PoseFilter.GetPositions();
        }
      }
      else
      {
        ...
        {
          return PoseLandmarks!= null ? PoseLandmarks.ToList() : null;
        }
      }
      return null;
    }
